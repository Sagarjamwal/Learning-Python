...
# --- 3. THE PROCESSING BRAIN ---
def generate_response(user_input):
    bot_response = ''
    
    # Add user input to our list temporarily to compare it
    ...
    try:
        ...
        # Find the best match (excluding the last one, which is the user input itself)
        ...
        bot_response = f"Error processing response: {e}"
        
    # Clean up: Remove user input so it doesn't get stuck in memory
    sentence_tokens.remove(user_input)
    
    return bot_response

# --- 4. THE CHAT LOOP ---
print("\nROBO: My name is Robo. I will answer your queries about Python. Type 'Bye' to exit.")
...
while flag:
    ...
    if user_response != 'bye':
        if user_response == 'thanks' or user_response == 'thank you':
            ...
            print("ROBO: ", end="")
            # Verify variable exists before calling function
            if 'sentence_tokens' in globals():
                print(generate_response(user_response))
            else:
                print("Error: Brain not loaded.")
            print("-" * 20)
    else:
        flag = False
        print("ROBO: Chat with you later!")